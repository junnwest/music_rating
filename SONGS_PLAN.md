...
4. ✅ `AddModal` song mode + song Instinct (2026-06-18). Migration `20260618000002_song_ratings.sql` (⏳ apply via SQL editor): `track_ratings.elo_score`/`elo_games` + `track_pairwise_comparisons` (songs compare only with songs). `AddModal` + `StarRatingWidget` now take a `RateTarget` (`album` | `song`) and branch storage/opponents/compare endpoint by kind; new `/api/rate/compare-song`. Song page + tracklist rows use the widget (album page tracklist row = compact: Add/Re-rank only; full Re-rank/Delete on the song page). **Add = rate**, `QuickAddButton` = Save (collections). ⚠️ **Gap:** the Manual→Instinct import (`/api/rate/seed-from-manual`) only seeds `ratings` (albums) — it does NOT yet seed `track_ratings` (songs), so a switcher's existing Manual song ratings don't carry into song Instinct. Extend the seed endpoint to `track_ratings` as a follow-up.
