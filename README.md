...
>
> **⚑ NEXT SESSION — PICK UP HERE (Windows left off 2026-07-08, sessions 3–4; Mac session 7 below):**
>
...
> - ✅→🔴 **Cover proxy is BROKEN on prod — was making iOS album covers permanently fail, not just slow. Client-side reverted 2026-07-08 (Mac session 7); server-side proxy itself is still broken and needs Windows's side-table fix.** `apps/web/app/api/img/route.ts` (caching proxy for slow Cover Art Archive covers) deployed but returns **502** — archive.org almost certainly blocks Vercel's datacenter IPs (confirmed live via curl: 502 in 0.8s, body `upstream error`, vs. direct CAA 200 in 2.4s). The edge-proxy shape is wrong. **Fix = resolve each CAA URL to its direct `ia*.archive.org` URL once from a local (allowed) IP, store in a side table, client fetches directly.** The iOS `Theme.swift` change (`31d2a1a`) had routed **~95% of covers** through this dead proxy — confirmed live in the simulator that affected covers sat on the placeholder indefinitely (8+ seconds, zero change), not just slowly. **Reverted `thumbnailUrl` to skip `/api/img` entirely** — CAA covers load direct again (~2.4s, slower than the proxy's intended instant-CDN-hit but at least working). Re-apply the proxy routing once Windows ships the resolved-URL side table; until then this revert should stay in place. Full detail in SESSIONS 07-06 + 07-08 cover entries.
