...
>
> 1. **Restart the pipeline.** Still the oldest open item — several 2026-07-11/12 fixes (discovery dedup, cross-source dedup guard, tier-2 Deezer miss-fallback, Q1 demand gates) only take effect on a fresh start, and nothing since indicates it's happened:
>    ```powershell
>    cd apps/web ; $env:DISCOVER_CEILING = "50000" ; npm run pipeline -- --no-embed
>    ```
>    Full context in the "ON RESUME" entry just below.
...
> 3. **Queue 2 confirmed catalog gaps** via the `seed:missing`-by-MBID workflow (see Data-Fix Track below): **Masta Wu** (zero artist row in the catalog at all, not just one missing release) and **Simon Dominic's "ONYX"** (missing despite his other 9 releases being present). Both confirmed via direct artist-scoped query this session, not just a search miss — real gaps, not a matching bug.
...
>
> **⚑ QUEUE FOR PIPELINE — 2 confirmed catalog gaps found while debugging the above (2026-07-14, Mac):** **Masta Wu is entirely absent** (zero artist row, not just one release) and **Simon Dominic's "ONYX"** is missing despite the rest of his discography (9 releases) being present. Both confirmed via direct artist-scoped query, not just a search miss. Good candidates for the `seed:missing`-by-MBID workflow (see Data-Fix Track below) next time the pipeline runs.
>
