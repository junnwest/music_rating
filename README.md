...
11. **Next (session 4):** Continue screen-by-screen pass — `RankingDetailView` (leaderboard for a category, tappable rows in `RankingsView`), `HomeView` UI polish, "Add" tab actual add-to-list UX, pull-to-refresh (needs `hasLoaded` reset). Also: run DB migrations (`rating_mode`, `elo_score`, `pairwise_comparisons`) to unblock onboarding end-to-end. Investigate `checkOnboarded` returning false for existing Spotify users (profiles row / RLS).
12. **Later:** `RankingDetailView.swift` (leaderboard for a category, `NavigationLink` in `RankingsView`), MusicKit integration (Apple login), Spotify `user-top-read` sync — see `WEB_PARITY.md` §3
...
|------|-------------|------|
| `20260618000002_song_ratings.sql` | `track_ratings.elo_score`/`elo_games` (Instinct for songs) + `track_pairwise_comparisons` table + RLS (song-vs-song comparisons) | ⏳ pending — run via SQL editor |
