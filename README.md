...
>
> **⚑ NEXT SESSION — PICK UP HERE (Windows left off 2026-07-10; Mac session 5 below):**
>
...
> - ✅ **Cover proxy FIXED (07-09, `086451e`)** — the 502 was Vercel *edge* egress being rejected by archive.org (serverless IPs get through); `/api/img` moved edge→nodejs runtime + retries transient CAA cold-failures. Covers render on web now (the CSP was also missing `https://*.archive.org` for CAA's final redirect hop). **07-10:** proxy further hardened — per-attempt 6s abort timeout, 3 attempts with backoff, edge-cached 404s, and total failure now 302-redirects the browser to the direct URL instead of dying with a 502. The side-table URL-resolution idea is no longer needed. **iOS covers are unblocked** — the `Theme.swift` proxy routing is safe to ship.
