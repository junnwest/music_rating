...
**Priority order: apply pending migrations first (they unblock avatar uploads + notifications), then iOS RankingDetailView.**
...
13. **Next (session 6):** Run pending migrations `20260619000001_profiles_avatar.sql` + `20260619000004_notifications.sql` in Supabase SQL editor; create `avatars` storage bucket. `RankingDetailView` (leaderboard for a category) + `NavigationLink` in `RankingsView`. Wire Report/Block no-ops in feed card menu.
14. **Later:** MusicKit integration (Apple login), Spotify `user-top-read` sync — see `WEB_PARITY.md` §3
...
| `20260619000004_notifications.sql` | `notifications` table + RLS (recipient read + system insert) | ⏳ apply via SQL editor |
...
| `20260619000001_profiles_avatar.sql` | `avatar_url` column on `profiles` | ⏳ apply via SQL editor |
