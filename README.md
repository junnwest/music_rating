...
> 2. ✅ **Grammy AOTY re-seeded (360/360, 0 failed)** — 340 with MBID, 20 pending. Reconcile updated 67 release_groups.
> 3. ✅ **Build check** — Cmd+B succeeded.
> 4. ✅ **Mercury Prize seeded (332/332)** — 86 release_groups now have `prestige_score`.
> 5. ✅ **All Korean + major prestige sources seeded (2026-06-28)** — 15 sources total. reconcile: updated=247, pending=452. Sources: `weiv_aoty`, `golden_disc_daesang`, `golden_disc_bonsang`, `mma_aoty`, `sma_album`, `izm_aoty`, `kma_aoty`, `mama_aoty`, `kha_hiphop`, `kha_rnb`, `rhythmer_hiphop`, `rhythmer_rnb`, `kr_masterpiece_100`. **Gaps:** Weiv 2020+ (stopped), IZM 2015–2022 (archive defunct).
> 6. ✅ **RankingBlock wired to real Silla leaderboard (2026-06-28)** — was reading `viewModel.topRated` (community avg); now self-contained with its own `get_silla_leaderboard` call, functional genre/country filters, spinner states. `apps/ios/sillajuku/sillajuku/Main/RankingsView.swift`.
> 7. ✅ **Grammy genre sources seeded (2026-06-29)** — 6 new sources: `grammy_rap` (1996–2026), `grammy_rnb` (1995–2026), `grammy_rock` (1995–2026), `grammy_alternative` (1991–2026), `grammy_pop_vocal` (1968+1995–2026), `grammy_dance_electronic` (2005–2026 partial). Data files in `apps/web/scripts/data/`. Cases added to `seed-external-scores.ts`.
> 8. ✅ **Windows prestige prompt DONE (2026-06-29 Windows)** — (a) queued **726 modern Western prestige artists** at priority → ingesting (catalog 16k→~71k release_groups); (b) **RS500** (`data/rs500.ts`, 500, hardcoded MB MBIDs) + **Pitchfork 10.0** (`data/pitchfork-perfect.ts`, 175) seeded; (c) **Grammy Dance/Electronic** 2009–2013 filled + 2018 corrected (Kraftwerk); (d) **Brit Album** (`data/brit-album.ts`, 222) seeded. `external_scores` 2,250→3,219. reconcile climbed 247→**409**, then started **timing out (57014)** under the heavy ingest write-load → **re-run `npm run prestige:reconcile` after the prestige queue drains** (will be much higher). Also fixed `padDate` (`??` partial-date crash) + cleanliness audit (catalog clean). See SESSIONS.md 2026-06-29 (Windows) entry. **All 5 commits LOCAL — not pushed.**
> 9. Profile > Stats subtab (iOS)
> 10. TestFlight setup once Apple Developer account is active (paid $99 on 2026-06-23)
>
