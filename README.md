...
>
> **⚑ NEXT SESSION — PICK UP HERE (Windows left off 2026-07-08, sessions 3–4; Mac session 5 below):**
>
...
> - 🔴 **OPEN — cover proxy is BROKEN on prod.** `apps/web/app/api/img/route.ts` (caching proxy for slow Cover Art Archive covers) deployed but returns **502** — archive.org almost certainly blocks Vercel's datacenter IPs (fast ~1s failure, not a timeout; SSRF guard works). The edge-proxy shape is wrong. **Fix = resolve each CAA URL to its direct `ia*.archive.org` URL once from a local (allowed) IP, store in a side table, client fetches directly.** The pushed iOS `Theme.swift` change routes covers through this 502 — **DO NOT build/ship iOS covers until the proxy works** (or guard the change). Full detail in SESSIONS 07-06 cover entry.
