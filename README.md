...
> 4. **(Query-level, not data) `search_release_groups` RPC hardcodes `release_group_type IN ('album','ep')`, silently excluding compilations and singles from every fuzzy search result.** Found via the earlier Spotify-row-matching bug (Fred again..'s "USB," a real catalog album, is typed `compilation` and was invisible to this RPC). Patched around client-side in iOS (`fetchRelease()` now tries an unrestricted exact match first, only falling back to this RPC) — but the RPC itself is still narrower than it should be for anyone else calling it (web search, future features). Worth widening the type filter at the source rather than relying on every caller to route around it.
