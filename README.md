...
> **✅ Genre-aware search shipped (2026-07-12, Mac) — "korean hiphop" and similar genre phrases now return real genre-tagged albums, not just literal title matches.** Search was purely lexical before this (confirmed: no LLM anywhere in the codebase, no genre-array matching in either search RPC — the web's embedding boost only re-ranks rows that already passed a lexical filter, never expands the candidate set). New `genre_query_aliases` table (seeded from the two genre taxonomies already in the codebase — `genre-categories.ts` and `_compute_primary_genre`'s family table, both previously dead code) blends genre-tagged albums into `search_release_groups`'s existing ranked results — same signature, **zero iOS/web client changes**, live server-side only. Took 3 live-tested fixes past the initial design: (1) a correlated per-row genre check timed out (57014) — restructured as an indexed JOIN + new trigram index (`_genres_text()`, an explicit `IMMUTABLE` wrapper, same class of fix `normalize_text()` needed); (2) still returned zero results once fast — turned out Korean hip-hop releases carry no nationality-specific genre tag at all (Simon Dominic = plain `["hip hop"]`), fixed by gating Korean/Japanese-scene phrases on `artists.country` in addition to genre; (3) a second timeout from short/generic filter substrings (`'rap'`) even with the trigram index — fixed with a `LATERAL` + per-filter `LIMIT`, the same pattern `get_taste_similar_releases` already uses for embedding seeds. Verified live: "korean hiphop" → Beenzino/G-Dragon/Jay Park/Epik High/Changmo; ordinary searches unchanged and fast (~0.4-0.5s); worst case (`"pop"`) completes without timeout (~1.6s) with exact title matches still ranked first. **Known, accepted limitations** (catalog data, not search bugs): K-pop groups with hip-hop co-tags bleed into Korean hip-hop results (same issue the `primary_genre` chart system was built to solve, but that only covers ~3,644 chart-eligible rows); a meaningful chunk of the Korean indie catalog (검정치마, most of dress) has `genres = NULL` entirely, unfindable by genre no matter how the query logic works. See SESSIONS.md 2026-07-12 for the full 3-round trace.
>
