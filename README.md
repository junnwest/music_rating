...
>
> **⚑ NEXT SESSION — PICK UP HERE (session left off 2026-07-06, Mac, session 3):**
>
> **Profile visibility overhaul + subtabs on other users' profiles** — see SESSIONS.md (2026-07-06, session 3). Settings' Privacy section is now a general Public/Private toggle + an Advanced section with independent Catalog/Library/Stats overrides ("Followers only" removed — Private now means followers-only). Real enforcement built for the first time via new RPCs (`get_profile_subtab_access` + 3 data RPCs, migrations `20260706000012`/`20260706000013`, **both applied**) — RLS itself was left untouched (found it's always been fully open). `UserProfileView.swift` rewritten with the same 3 subtabs + sort/filter/display-mode controls as the owner's own profile. Two Xcode build errors hit and fixed (a ternary the type checker couldn't resolve, then a compiler crash confirming it was real) — **not yet successfully rebuilt after the second fix.**
>
> **Not yet live-tested, still open (this session):**
> 1. Rebuild in Xcode — confirm the compiler-crash fix actually resolved it.
...
> - ⚑ **Flag for Windows, not yet actioned:** three independent external-LLM reviews (drafted a prompt with live DB numbers, sent to 3 models) converged on the same root fix — bot-seeding currently over-indexes on already-popular albums (top individual counts: 42, 41, 40...; only 19% of the prestige pool touched at all despite 8,057 ratings). The fix is seeding *order*, not more volume: prioritize zero/low-coverage prestige-pool albums before re-rating popular ones. This is what makes the Rankings gauges above converge in reasonable time instead of requiring an ever-higher N to compensate for skew.
> - ⚑ **Also flagged for Windows, not fixed:** Rihanna's `artists` row has `name_native = '리아나'` and `native_language = 'ko'` — wrong data (she's not Korean-native), same bug class as the E SENS fix from 2026-07-03. Root-caused, not corrected — user chose to route it to Windows.
