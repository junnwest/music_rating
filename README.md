...
> - ⏳ **NEXT: the bot SOCIAL pass** (bot→bot follows/likes/few comments — currently 6/8/1). ⚠️ `rating_likes`/`rating_comments`/`follows` inserts → notification triggers → **pg_net→APNs push webhook**; must be **bot-on-bot-only** (don't push real users fake engagement), bounded, null-`push_token`-verified. Code changes (`bot-reviews.ts`, `generate-bot-ratings.ts`) **not yet committed**.
>
