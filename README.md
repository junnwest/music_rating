...
> **⚑ APP STORE REJECTION FIXED, RESUBMISSION PENDING (2026-07-18, Mac).** Apple rejected the review build (Guideline 4: Sign in with Apple must not ask for the name it already provides — onboarding's "What's your name?" step). Fixed in `OnboardingView`: the name step is now skipped whenever the OAuth provider supplied a name (metadata `full_name`/`name`, resolved in `init`), any provider; shown only when no name is available. Build number bumped to **7**. Verified on-sim. **User to do:** archive + upload build 7 via Xcode Organizer (no ASC API key on this machine), reply to the App Review message, resubmit. Ships together with the full 2026-07-17 Instagram share-background feature set (all still uncommitted).
