...
> **✅ NEW (2026-07-10, Windows) — coverage verification harness + romanization-aware matching fix. `npm run verify:coverage` (READ-ONLY) measures how well the catalog matches real iTunes/Spotify discographies (the same source users' connected data comes from), classifying every miss as catalog-gap vs match-bug. Baseline found the biggest bug: `resolveArtist`/`fetchRelease`'s client gate only checked native `name`/`name_native`, so a CJK-named artist matched via a romanized ALIAS (혁오 via "HYUKOH") was thrown away — broke Spotify/Apple matching for most of the target audience. Fixed (`20260710000001` applied live: search_artists returns `aliases[]` + alias-exact ranking; search_release_groups returns `primary_artist_id`; iOS gates + harness consume them). Artist resolution 79.5% → 95.5%, mainstream floor unchanged (no false positives). ⚠️ iOS unbuilt — needs a Mac build. Full trace in SESSIONS.md 2026-07-10. Three residual workstreams the harness now measures: (1) edition/punctuation normalization ("SCARING THE HOES: DIRECTOR'S CUT" vs "…", "1991-1994" vs "1991–1994") — 32 remaining match-bugs; (2) accent folding (Blase vs BLASÉ); (3) real coverage gaps (JP ~34%) → `catalogGapArtists` MBIDs in `scripts/output/coverage-after-fix.json` are the goal-directed seed list for the pipeline. STILL OPEN: `CRON_SECRET` on Vercel (only 1/167 profiles has taste data — cron isn't running in prod), and the durable stale-name fix at `mb-ingest.ts:206`.**
>
