...
>
> **Still open, ready to apply:** Supabase's Performance Advisor surfaced two things this session drafted migrations for:
> - **Fixed, migration ready to apply:** `public.critic_affiliation` view had no `security_invoker`, meaning it checked the view owner's RLS/permissions instead of the querying user's (Critical). Migration `20260708000000_critic_affiliation_security_invoker.sql` is written and verified safe (both underlying tables have wide-open `SELECT USING (true)` policies) — just needs applying via SQL editor.
> - **Fixed, migration ready to apply:** a full audit (every `CREATE POLICY`/`ALTER POLICY` across both migration directories, traced chronologically to each table's currently-live definition) found **60 policies across 27 tables** calling `auth.uid()`/`auth.role()` unwrapped in `USING`/`WITH CHECK` — Postgres re-evaluates these per-row instead of once per query, a systemic tax on every authenticated request (not tied to the acute CPU incident above specifically, just found via the same advisor pass). `20260708000001_rls_auth_initplan_fix.sql` fixes all 60 via `ALTER POLICY` (preserves grants/ordering, no drop window). Verified each clause directly against its source migration before writing the fix (not just the advisor's summary) — three of the EXISTS-based ones (`list_item_tracks`, `user_ranking_entries`, `mix_items`/`mix_likes`/`mix_shares`) needed the real join shape, not a guessed one. Also worth knowing: for any policy that was `FOR ALL`/`FOR UPDATE` with only a `USING` clause at creation, Postgres auto-copies it into `WITH CHECK` — the migration explicitly restates both (16 such policies) rather than leaving that copy stale.
>   - **Not addressed** (flagged in the migration's header, needs a judgment call, not a mechanical fix): 6 tables (`mixes`, `mix_items`, `mix_likes`, `mix_shares`, `mix_share_likes`, `rating_likes`) each have both a `FOR ALL` (owner) and a separate `FOR SELECT` (public/broader) permissive policy — not redundant (each expresses a different visibility rule), but Postgres evaluates both per row, doubling cost even after this fix. `daily_answers`/`daily_questions` are defined twice across the two migration directories with different policy names — the fix migration assumes `apps/web`'s naming is what's live; if that's wrong its `ALTER POLICY` statements will fail loudly (safe failure mode, not silent). `list_item_tracks` has no migration at all under `apps/web` (only the top-level dir) — fixed anyway since `ALTER POLICY` targets the live table regardless of which directory tracks it, but worth reconciling separately. Optional bonus fix for `storage.objects` avatar policies (same anti-pattern, different schema) is included commented-out at the bottom of the same file.
>   - **Real, confirmed, NOT fixed here — needs Windows/dashboard access:** the original advisor paste the user grabbed showed `ratings` with policies named `"Allow authenticated users to select/insert/update/delete their ratings"` **in addition to** the tracked `ratings_select`/`ratings_insert`/`ratings_update`/`ratings_delete`. Those `"Allow authenticated..."` names **don't appear in any migration file in either directory** — grepped both, zero hits — so they're live but untracked (same SQL-editor-only blind spot as `reconcile_prestige_scores()`). That means `ratings` likely has **7 overlapping policies live**, not the 4 this migration's `ALTER POLICY` statements can see, and per-row auth-check cost on `ratings` is probably double what fixing the tracked 4 alone will achieve. Couldn't fix these blind — don't have their exact `USING`/`WITH CHECK` text and guessing at security-relevant SQL is exactly the kind of thing not to guess at. **Action needed:** run `select policyname, cmd, qual, with_check from pg_policies where tablename = 'ratings';` in the SQL editor; if the untracked ones are functionally superseded by the tracked short-named ones, just `DROP POLICY` the untracked ones (consolidates AND fixes the wrap issue in one move, no need to reconstruct their exact clause). Worth spot-checking a few other high-traffic tables the same way in case this isn't unique to `ratings`.
>
