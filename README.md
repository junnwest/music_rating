...
>
> **⚑ NEXT SESSION — PICK UP HERE (session left off 2026-07-01, Mac):**
>
> **What was built this session (Mac iOS — 2026-07-01):**
> - ✅ **Delete account fix** — `Config.webBaseURL` → `https://www.sillajuku.com` (Vercel redirect stripped `Authorization` header on non-www calls). `authGuard.ts` now uses `supabase.auth.getUser(token)`. Debug logs removed.
> - ✅ **`ratings.review_text` column** — migration `20260701000001_ratings_review_text.sql` ✅ applied in Supabase.
> - ✅ **Post-rating comment + list flow** — `PostRatingOptionsView.swift` (new — inline comment field, no immediate save; Add to list via MixPickerView). Instinct mode: stores `pendingReviewText`, saves in `finalize()`. Normal mode: `ManualRatingSheet` transitions to postRating phase after save. `HomeView` FeedCard shows `review_text` above the action bar.
> - ✅ **Profile posts display mode** — `ProfileView` has a list/posts toggle in the ratings filter row. Posts view shows `ProfilePostCard` (cover + score chip + review text + likes/comments/date). Bulk-fetches counts. `ReleaseRef.typeLabel` added.
> - ✅ **Delete ratings from profile** — long-press context menu ("Delete Rating") on both list rows and post cards. Optimistic local remove + deletes from `ratings` (albums) or `track_ratings` (songs).
>
> **Open — pick up next session:**
> 1. **Profile > Stats subtab (iOS)** — not yet built.
> 2. **TestFlight setup** — Apple Developer account active ($99 paid 2026-06-23). Need to create App ID + provisioning profile + upload first build.
...
> 4. **`npm run prestige:reconcile`** — re-run after the prestige artist ingest queue drains (was timing out under heavy load at 409 reconciled; will be much higher once stable).
>
