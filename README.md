...
**Priority order: apply `20260620000002_charts_rpcs.sql` + `20260620000003_charts_song_rpcs.sql` migrations (unblocks Charts data), then create `avatars` bucket in Supabase Storage.**
...
18. **Next:** Apply `20260622000001_spotify_data_cache.sql` first (unlocks Spotify DB persistence), then `20260620000002_charts_rpcs.sql` + `20260620000003_charts_song_rpcs.sql`. Create `avatars` storage bucket in Supabase Storage → re-enable avatar uploads in `EditProfileView`. Wire Report/Block no-ops in feed card menu. Profile > Stats subtab. Complete TestFlight setup once Apple Developer account is active.
19. **Later:** MusicKit integration (Apple login), Spotify `user-top-read` sync — see `WEB_PARITY.md` §3
...
|------|-------------|------|
| `20260622000001_spotify_data_cache.sql` | `spotify_artists jsonb`, `spotify_recently_played jsonb`, `spotify_data_updated_at timestamptz` on `profiles` — persistent Spotify data cache; survives token expiry, reinstalls, and device switches | ⏳ apply via SQL editor |
...
| `20260620000003_charts_song_rpcs.sql` | 3 song chart RPCs: `get_charts_top_rated_songs`, `get_charts_most_rated_songs`, `get_charts_trending_songs` | ⏳ apply via SQL editor |
| `20260620000002_charts_rpcs.sql` | 8 SECURITY DEFINER RPCs for the Charts tab: `get_charts_pulse`, `get_charts_top_rated` (min 1 rating, was 5), `get_charts_most_rated`, `get_charts_trending`, `get_charts_trending_for_genres`, `get_user_top_genres`, `get_charts_hidden_gems`, `get_charts_controversial` | ⏳ apply via SQL editor (re-run if already applied — threshold changed) |
