...
>
> **⚑ NEXT SESSION — PICK UP HERE (session left off 2026-07-03, Mac):**
>
...
> - ✅ **Korean phonetic search (`드레이크`→Drake) — infra built.** New `artists.name_phonetic_ko` column (a *separate* column, deliberately not overloading `name_native`, per the handoff — `name_native` = an artist's actual native identity, `name_phonetic_ko` = the searchable Korean spelling of a non-Korean artist). Migration `20260703000006_artist_phonetic_ko.sql` adds the column + trigram index and rebuilds `search_artists` to match/rank it (RETURN signature unchanged → no iOS/web client changes). Backfill `scripts/backfill-phonetic-ko.ts` (`npm run backfill:phonetic-ko`) sources the rendering from Korean Wikipedia interlanguage links — public API, no scraping. Validated live: Drake→드레이크, Taylor Swift→테일러 스위프트, The Weeknd→위켄드, Kendrick Lamar→켄드릭 라마, Radiohead→라디오헤드, Coldplay→콜드플레이; conservative (returns no-match rather than a wrong guess on ambiguous pages). **⏳ Pending: apply migration `20260703000006`, then run the backfill** (~21k non-Korean-native artists, famous-first; Wikipedia-bound, DB-write-light).
> - ⚠️ **Korean native album titles — source research done, blocked on a decision.** Re-confirmed both documented dead-ends (iTunes KR returns the Korean *artist* name but *English* album titles, e.g. 뉴진스 :: "NewJeans 2nd EP 'Get Up'"; MusicBrainz carries no Korean RG titles) and ruled out the clean alternatives (Wikidata has ~0 K-pop album coverage; Deezer's Korean search is unreliable — wrong-artist matches). The only reliable source is the Korean streaming platforms (Melon/Bugs/Genie), all **scraping with ToS implications** — escalated per the handoff rather than proceeding. Gap size: **6,464 / 7,425 (87%)** of Korean-artist release groups lack `native_title`.
>
...
> - **Still open from the 23-finding audit** (deliberately not attempted this pass — each needs either a new external account/service the user would need to set up, or is a large enough scope to warrant its own dedicated session): zero automated tests, no staging environment, no uptime/health alerting, no backup *verification* (this pass added the backup itself, not a tested restore drill), unstructured logging, iOS accessibility support, deep linking/Universal Links, lifecycle/transactional email, feature flags (PostHog already supports them, just nothing uses it yet), sitemap/structured data (correctly deferred pre-launch per the audit itself).
>
