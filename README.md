...
13. **Next (session 6):** Create `avatars` storage bucket in Supabase Storage → re-enable avatar uploads in `EditProfileView`. `RankingDetailView` (leaderboard for a category) + `NavigationLink` in `RankingsView`. Wire Report/Block no-ops in feed card menu.
