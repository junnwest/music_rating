...
- **⚠️ GAPFILL job C (skipped-artist iTunes recovery) DISABLED** by default (`GAPFILL_RECOVER_ARTISTS=1` to re-enable) — it created store-country shadow artists that duplicated MB rows; covers + tracklists only. MB-skipped generic names → recover via `mb-overrides` instead.
