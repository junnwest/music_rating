...
**✅ DB RENOVATION COMPLETE + MB CATALOG PIPELINE BUILT & RUNNING (2026-06-26, Windows):**

The renovation migration is applied. Catalog re-ingestion is now a **single long-running orchestrator** built on **MusicBrainz** (CC0, MBID identity) — *not* iTunes. Run it: **`cd apps/web && npm run pipeline`** · status: **`npm run pipeline:status`**. (The iTunes catalog-expansion sections further down this README are **superseded** by this MB pipeline.)
...
- **DISCOVER** — seeds the ~275 curated artists once ([`seed-artists.ts`](apps/web/scripts/seed-artists.ts)). Breadth is currently **manual**: `npm run mb:discover` (ListenBrainz similar, CC0) + `npm run queue:build:global` (Wikipedia).

**Tooling:** `npm run mb:audit` (read-only integrity), `mb:ingest:one`, `mb:embed`.

**⚠️ This is the core engine — NOT yet the "run-for-a-week, auto-decides-all-jobs" system. Next build order:**
1. **Self-feeding DISCOVER lane** *(biggest gap)* — when `pending` runs low, auto top-up the queue (bounded LB + Wikipedia) so it **grows over a week** instead of idling after the seed drains.
2. **iTunes GAPFILL** (append-only for MB misses) + populate [`mb-overrides.ts`](apps/web/scripts/mb-overrides.ts) (~8 generic names: dean, gray, kai, txt, a pink, loco, woo, miso — currently skip as `needs_review`).
3. **FRESHNESS lane** (re-poll `tracks_done` artists for new releases) + **QC lane** (automate `mb-audit` + auto-requeue failures).
4. **Rating re-link** (restore `backups/` 98+20 ratings onto new `release_group_id`s) + **chart RPC rebuild** (dropped in renovation; song RPCs will key on `recording_id` — coordinate with iOS).

**iOS schema (for the Swift rewrite, delivered 2026-06-26):** rate **`release_group_id`** (not release_id); songs key on **`recordings.id`** (uuid); `release_groups.release_group_type` is lowercase; artist field is **`artist_display`** (recordings too — old `artists` is gone); track loading = canonical `releases` (`is_canonical=true`) → `release_tracks` join `recordings` (no view yet); PostgREST embeds `ratings→release_groups` / `mix_items→release_groups` **auto-detect** (no FK hint). Chart song RPCs **not rebuilt yet**.

Rating backups saved to `backups/` (98 album ratings, 20 track ratings, gitignored).

**Mac priority order:** ✅ Report/Block wired (2026-06-25) → ✅ **All 9 Swift files updated to new DB schema (2026-06-26)** → Build/test on device → Profile > Stats subtab → TestFlight setup.
...
22. **Next:** Build/test on device (confirm app compiles against new schema). Profile > Stats subtab. Complete TestFlight setup once Apple Developer account is active. ✅ `avatars` storage bucket confirmed created.
