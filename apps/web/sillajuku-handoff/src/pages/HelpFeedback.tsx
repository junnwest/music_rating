...
const faqs = [
  ...
  },
  {
    ...
    a: 'Go to any user\'s profile and click the "Follow" button. You can also discover people through the Friends page > Discover tab.',
  },
  {
    q: 'What is Wrapped?',
...
export default function HelpFeedback() {
  ...
  const [email, setEmail] = useState('kenneth@example.com');
    ...
    <div className="flex-1">
      ...
      <div className="max-w-[720px] mx-auto px-5 py-10 pb-20 w-full">
        ...
        <section className="mb-14">
          <h2 className="text-[13px] font-bold text-muted uppercase mb-5" style={{ letterSpacing: '0.7px' }}>Quick Help</h2>
            ...
            {filteredFaqs.map((faq, i) => (
              <div key={i} className="border border-divider rounded-xl bg-white overflow-hidden">
                ...
                >
                  ...
                  <ChevronDown
                    size={16}
                    className={`text-muted flex-shrink-0 transition-transform ${openFaq === i ? 'rotate-180' : ''}`}
                  />
                </button>
                <AnimatePresence>
                  {openFaq === i && (
                    ...
                    >
                      <p className="px-4 pb-4 text-[13px] text-mid leading-relaxed">{faq.a}</p>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
          ...
          {filteredFaqs.length === 0 && (
            <p className="text-[13px] text-muted text-center py-8">No matching questions. Try another search or send us a message below.</p>
          )}
        </section>

        {/* ── Contact Form ── */}
        <section className="mb-10">
          <h2 className="text-[13px] font-bold text-muted uppercase mb-5" style={{ letterSpacing: '0.7px' }}>Contact Us</h2>

          {submitted ? (
            <motion.div
              initial={{ opacity: 0, y: 8 }}
              animate={{ opacity: 1, y: 0 }}
              className="border border-mint rounded-2xl p-6 bg-mint-bg"
            >
              <div className="flex items-center gap-3 mb-2">
            ...
            <div className="border border-divider rounded-2xl p-6 bg-white">
                  ...
                  {categories.map(c => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-full text-[11px] font-semibold border-2 transition ${
                        category === c ? 'border-mint bg-mint-bg text-mint-dark' : 'border-divider text-muted hover:border-ink hover:text-ink'
                      }`}
                    >
                      {c}
                    </button>
              ...
              {/* Message */}
              <div className="mb-4">
                <label className="block text-[13px] font-semibold text-ink mb-2">Message <span className="text-red-500">*</span></label>
                <textarea
                  rows={5}
                  value={message}
              ...
              {/* Email */}
              <div className="mb-5">
                <label className="block text-[13px] font-semibold text-ink mb-2">Email <span className="text-muted font-normal">(optional)</span></label>
                <input
                  type="email"
                  value={email}
