-- Raise word_similarity threshold 0.4 → 0.5.
-- Eliminates false positives where short prefix overlap triggers fuzzy matching:
--   "Frank Ocean" → "Pink Ocean"  (word_similarity ≈ 0.42)
--   "Vaundy"      → "Vault"       (word_similarity ≈ 0.43)
-- Typo tolerance on longer strings (Blakpink, Kendrik Lamar) is unaffected.
...
LANGUAGE sql STABLE AS $$
  ...
  WHERE
    ...
    AND (
      ...
      OR word_similarity(lower(q), lower(r.title))  > 0.5
      OR word_similarity(lower(q), lower(r.artist)) > 0.5
      OR (r.title_native  IS NOT NULL AND word_similarity(lower(q), lower(r.title_native))  > 0.5)
      OR (r.artist_native IS NOT NULL AND word_similarity(lower(q), lower(r.artist_native)) > 0.5)
