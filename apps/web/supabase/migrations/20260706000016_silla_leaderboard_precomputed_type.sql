-- Restore the 20260705000002 precomputed-prestige fix that
-- 20260706000002_charts_release_type.sql accidentally clobbered.
--
-- What happened: 20260706000002 rebuilt every chart RPC "from its current
-- live definition" to add release_group_type — but for get_silla_leaderboard
-- it copied 20260703000004's live-computation body (global_prestige +
-- all_prestige computed from external_scores on every call, joined against
-- ~295k release_groups, no plan hints). That body is the exact shape the
-- 2026-07-05 timeout saga (20260704000000 → 20260705000000/1/2) took four
-- rounds to eliminate; the durable fix lives in the ROOT-level
-- supabase/migrations/20260705000002_silla_leaderboard_precomputed.sql,
-- which apps/web/supabase/migrations doesn't contain — easy to miss.
-- Net effect: get_silla_leaderboard consistently times out again (57014,
-- verified 2026-07-06 from the web client under both anon and service_role),
-- which blanks the Ranking block on web /charts and iOS ChartsView.
--
-- This migration re-applies the 20260705000002 body verbatim (global path
-- reads the precomputed, indexed release_groups.prestige_score — no join;
-- country path keeps live all_prestige computation with enable_nestloop=off)
-- and ONLY adds release_group_type to RETURNS TABLE + the SELECT lists,
-- preserving 20260706000002's intent. Safe to re-run.
