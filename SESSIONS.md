...
- **Three residual issues the harness now makes measurable (each a separate workstream):** (1) **Edition/punctuation normalization** — 32 remaining match-bugs are almost all edition variants ("SCARING THE HOES: DIRECTOR'S CUT" vs "SCARING THE HOES") or CJK-dash variants ("1991-1994" vs "1991–1994"); the matcher (and `stripItunesSuffix`) doesn't fold these. (2) **Accent folding** — "Blase" (query) ≠ "BLASÉ" (catalog); normalize_text keeps the accented é. (3) **Real coverage gaps** — Japanese especially (34% album match); the report's `catalogGapArtists` MBID list is the seed set for the pipeline.
