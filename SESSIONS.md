...
- **Profile share URL fixed (iOS):** `ProfileView.profileURL` was generating `/@username` (404). Fixed to `/profile/username`. `ProfileShareItem.activityViewControllerLinkMetadata` continues to set the flower logo as the iOS share sheet thumbnail.
- **`/@username` redirect (web):** Added `{ source: '/@:username', destination: '/profile/:username', permanent: true }` to `next.config.mjs` redirects — handles any old shared links using the broken format.
- **Taste page (also session 15, missed from previous entry):** Fixed "Rate X more albums to unlock Taste" → "releases". Added "Find releases to rate" `Button` below the `0 of 25` counter in `TasteLockView`, guarded by `onGoToAdd != nil`. `TasteView` gained `var onGoToAdd: (() -> Void)? = nil`; `MainTabView` passes `{ selectedTab = .add }`.
- **OG images overhaul (web):** Replaced all custom OG image edge functions with simpler approach — set `og:image` directly in `generateMetadata` to the content's own image URL. `profile/[username]`: `avatar_url` from profiles (falls back to no image); `album/[mbid]`: `cover_url` from releases (queries by `id` or `spotify_id`); `song/[trackId]`: parent release `cover_url`. Twitter card set to `summary` for profile (square avatar), `summary_large_image` for album/song (wide cover).
- **Main page OG image (web):** Simplified `app/opengraph-image.tsx` to render the flower logo (PNG, extracted from `logo-flower.svg`) centered on white. No `theme-color` set — iMessage card shows flower on default gray/white background.
- **New asset:** `logo-dense.svg` (new wordmark with custom dense paths) added to `apps/web/public/`. `logo-flower.png` extracted from the embedded base64 PNG in `logo-flower.svg` and added to `apps/web/public/` for use in `ImageResponse` (Satori cannot render SVGs that embed other images).
