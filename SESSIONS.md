...
- **Archived and exported three builds in sequence** (`xcodebuild archive` + `-exportArchive`, `app-store-connect` method, automatic signing, team `GGJ5HX3A4M`) as prep work accumulated: build 3 → Xcode silently bumped the *exported* artifact to build 4 during export (confirmed with the user: they'd manually archived a build 3 a few days earlier outside this session, so Xcode's own App Store Connect conflict check was correct) → synced `CURRENT_PROJECT_VERSION` to 4 to match. Build 4 uploaded and processed successfully by the user via Xcode Organizer (dSYM-for-Sentry warning on upload — same known-benign issue as Build 2, Sentry's XCFramework ships with no bundled dSYM at all).
- **iPad support pulled for this submission — found genuinely broken, not just unadapted.** The app declares `TARGETED_DEVICE_FAMILY = "1,2"` but has never been run or tested on iPad this whole project. Set up two purpose-built simulators (`iPhone 17 Pro Max` and `iPad Pro 13-inch (M5)`, freshly created — matches Apple's exact 6.9"/13" screenshot size classes, confirmed pixel-for-pixel via `sips`: 1320×2868 and 2064×2752, no resizing needed) for App Store screenshot capture. The iPad screenshot immediately showed a real defect: the top tab bar overlaps the first feed post's content, and the feed is just the iPhone single-column list stretched full-width with no layout adaptation — not cosmetic rough edges, an actual overlap that could hide content and is exactly the kind of thing App Review tests explicitly on iPad-declared apps. Given a proper responsive iPad pass isn't a same-day task, user chose to ship iPhone-only for now: `TARGETED_DEVICE_FAMILY` → `"1"`, bumped to **build 5**, re-archived (confirmed `UIDeviceFamily` in the exported Info.plist is now `[1]` only, no silent build-number bump this time since 5 was already the correct next number) — this is the build going to App Store Connect for actual submission. iPad support is now a real, deliberate future project instead of a rushed same-day patch. (Screenshot capture itself — for iPhone, in both English and Korean per the user's choice — is the next in-progress task, picking up from here.)
