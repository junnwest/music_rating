...
- 🔴 **The `primary_genre` bulk backfill WAS the Mac's "07-07 open DB-load incident"** — my PostgREST backfill IO-starved the shared Micro instance and degraded live prod (couldn't be attributed to Windows because it wasn't pushed). **Stopped; will not restart against the live instance.** The bulk write (~250k rows still null) must run off-peak / heavily-throttled, or via the server-side `CALL backfill_primary_genre_proc()` on a coordinated low-traffic window.
