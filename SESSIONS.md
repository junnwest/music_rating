...
- **Migration `20260706000001_primary_genre.sql`** — adds `primary_genre` column + `_rg_primary_matches()` helper (falls back to whole-array while null, so no empty-chart window), and swaps the genre filter in the 4 genre charts (`get_charts_top_rated`/`most_rated`/`hidden_gems`/`trending_for_genres`) to match primary only. Reproduced from each RPC's *live* body (top_rated from `20260705000005`, rest from `20260626000002`). ⚠️ **Post-merge (07-07): the Mac's parallel web-rebuild + live RPC fixes (000016/000017) may have re-created these chart RPCs from a `_rg_has_genre` base — the live filter state must be re-verified before this counts as active.** A server-side `_compute_primary_genre` + resumable `backfill_primary_genre_proc()` (`20260706000002`) were added; the PostgREST backfill was the "07-07 open DB-load incident" (IO-starved the Micro instance / prod), now stopped — the bulk write must wait for a fresh IO budget.
- ⚠️ **`get_silla_leaderboard` deferred** — same leak, its live body was never committed as a migration.
- **Also this session:** a caching cover proxy (entry above) and a merge of the Mac's 21-commit push (web rebuild + sessions 4–5).
