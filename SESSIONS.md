...
- **Not yet done:** wiring the recency lane into the pipeline loop as a scheduled lane (it's a manual/cron script for now — it shares iTunes with the gapfill lane, so it must stay bounded); back-linking the resolved `itunes_artist_id` on ingest (skipped to avoid any dup-artist path — future runs re-resolve via album search). `tsc` clean across all changes.
