...
**2026-07-12 (Mac) — Genre-aware search: "korean hiphop" now returns real Korean hip-hop albums, not just literal title matches (took 3 live-tested fixes to get right):**
...
**Three real bugs found and fixed via live testing, not assumed fixed from the design alone:**
...
3. **Second timeout after the country gate.** Some broadened filter substrings (`'rap'` alone) were short/common enough that even the new trigram index returned too large a candidate bitmap before `DISTINCT ON`/`LIMIT` could bound it. Fixed by restructuring to a `LATERAL` join with a per-filter `ORDER BY ... LIMIT` — the exact same pattern `get_taste_similar_releases` already uses for its per-seed embedding lookups (2026-07-10), applied per genre filter instead of per seed. Also trimmed the riskiest short substrings (`'rap'`, `'pop'` inside the indie bucket) back to only what's actually confirmed present in real tag data.

**Verified live end-to-end**, not just design-reviewed: "korean hiphop" → Beenzino, G-Dragon, Jay Park, Epik High, Changmo; "shoegaze" → My Bloody Valentine's *Loveless*, Wolf Alice, The Cure; "korean r&b" → Crush. Ordinary searches (artist names, album titles) confirmed unchanged and fast (~0.4-0.5s). Worst case tested (`"pop"`, the broadest single-word alias) still completes without timeout (~1.6s) and correctly keeps literal exact-title matches (U2's actual album *Pop*) ranked above genre-blended results.

**Known, accepted limitations (not bugs — catalog data reality):** K-pop groups with hip-hop-tagged co-tags (BTS, NCT DREAM, MONSTA X) surface alongside dedicated hip-hop artists for genre-blended Korean hip-hop results — the same generic-co-tagging pattern that motivated the separate `primary_genre` precedence system, which only covers the ~3,644 chart-eligible rows, not the whole catalog. Separately, a meaningful chunk of the Korean indie catalog (검정치마/The Black Skirts, most of dress's discography) has `genres = NULL` entirely — no genre-matching strategy can surface a release with no tag data, regardless of how the query logic is built.

Migrations: `20260712000000` (table + first `search_release_groups` version) → `000001` (GIN array index, now dead weight for this feature specifically, harmless) → `000002` (timeout fix + `_genres_text()`) → `000003` (trigram index) → `000004` (country gate) → `000005` (LATERAL bound + substring trim, current live version).
