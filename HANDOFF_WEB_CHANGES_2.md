...
3. **Pipeline check:** a migration was applied this session (`20260926000003`, then `...0000` at the time), so per CLAUDE.md run `pipeline:status` and `pipeline:verify` if the pipeline is running, and log the result in `PIPELINE_CHECKS.md`. (It only adds functions and an index on `reports`; the pipeline's write path is untouched.)
...
**P5 / P6 / P7**
- Migration **`20260926000003_album_song_comments.sql` (applied; renumbered from ...0000 at merge)** adds:
